...
func (app *application) ServeHTTP(ctx context.Context) error {
	...
	server := &http.Server{Addr: app.config.HTTP.Addr, Handler: app.routes(), ReadTimeout: app.config.HTTP.ReadTimeout, WriteTimeout: app.config.HTTP.WriteTimeout, IdleTimeout: app.config.HTTP.IdleTimeout}
